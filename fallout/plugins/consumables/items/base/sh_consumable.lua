...
--[[
--addictions
ITEM.addictMax = 250 --how much the addiction increases per usage
ITEM.addictAmt = 10 --how much the addiction increases per usage
ITEM.addictData = {
	--function for when drug is used
	buffScale = 0.01, --how much 1 point of addiction adds to buff multiplier
	baseCD = 3600, --cooldown on forced symptom /me, gets reduced by addiction level
	
	--symptoms appear as addiction level goes over the threshold
	symptoms = {
		[15] = {
			"twitches slightly",
			"briefly zones out",
...
local function stomachCheck(item, client, char)
	if(nut.config.get("cookfood_stomach", true)) then
		...
		if(!stomachOverwrite) then
			if(char:getData("stomach", 0) < 20) then
		...
		if (curChar and curChar:getID() == charID) then
			...
			if (skills) then
				for k, v in pairs(skills) do
					curChar:removeSkillBoost(item:getName(), k)
				end
			end
...
local function consume(client, item)
	...
	local cure = item:getData("cure", item.cure)
	if(cure) then
		local buffs = table.Copy(client:getBuffs())
		local buffsToRemove = {}
		for k, v in pairs(buffs) do
			if(v.effect == cure) then
				client:removeBuff(nil, k)
			end
		end
	end
	...
	if(item.hp) then
		...
		timer.Create(id, tickRate, ticks, function()
			...
			local combatHP = math.Clamp(client:getHP() + 1, 0, client:getMaxHP())
			client:setHP(combatHP) -- for combat system
		end)
	end
...
ITEM.functions.usef = { -- sorry, for name order.
	...
	onRun = function(item)
		...
		if (IsValid(target) and target:IsPlayer() and target:Alive()) then
			if(target:GetNW2Bool("pac_in_editor")) then
				client:notify("You cannot feed people who are in PAC.")
				return false
			end
