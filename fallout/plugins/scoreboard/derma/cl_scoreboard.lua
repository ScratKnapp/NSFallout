local PANEL = {}
	...
	function PANEL:Think()
		if ((self.nextUpdate or 0) < CurTime()) then
			...
			for k, v in ipairs(self.teams) do
				local factionTbl = nut.faction.indices[k]

				visible, amount = v:IsVisible(), team.NumPlayers(k)
				playerCheck = false

				local localChar = LocalPlayer():getChar()
				
				for _, player in pairs(team.GetPlayers(k)) do
					local char = player:getChar()
					if(LocalPlayer() == player or localChar:doesRecognize(char)) then
						playerCheck = true
				...
				if (visible) then
					if(factionTbl and factionTbl.hidden) then
						if(!LocalPlayer():IsAdmin() or !playerCheck) then
							v:SetVisible(false)
							self.layout:InvalidateLayout()
						end
					elseif(amount == 0 or !playerCheck) then
						v:SetVisible(false)
						self.layout:InvalidateLayout()
					end
				end
				...
				if (!visible) then
					if(factionTbl and factionTbl.hidden) then
						if(LocalPlayer():IsAdmin() and playerCheck) then
							v:SetVisible(true)
							self.layout:InvalidateLayout()
						end
					elseif(amount > 0 and playerCheck) then
						v:SetVisible(true)
						self.layout:InvalidateLayout()
					end
				end
	...
	function PANEL:addPlayer(client, parent)
		...
		if (!char or !IsValid(parent)) then
			return
		end
		...
		if(client != LocalPlayer()) then
			if(!LocalPlayer():IsAdmin()) then
				if(localChar and !localChar:doesRecognize(char)) then
					return false
				end
			end
		end
...
hook.Add("CreateMenuButtons", "nutScoreboard", function(tabs)
	tabs["Player List"] = function(panel)
