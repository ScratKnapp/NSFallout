...
nut.command.add("doorbuy", {
	onRun = function(client, arguments)
		...
		-- Check if the entity is a valid door.
		if (IsValid(entity) and entity:isDoor() and !entity:getNetVar("disabled")) then
			local ownerTbl = entity:getNetVar("permaOwner") --can't buy if it has a perma owner
			if(ownerTbl) then
				return false
			end
			...
			local price = entity:getNetVar("dailyCost") or entity:getNetVar("price", nut.config.get("doorCost"))
...
nut.command.add("doorsetowner", {
	adminOnly = true,
	syntax = "[string name] <number days>",
	onRun = function(client, arguments)
		-- Get the door the player is looking at.
		local entity = client:GetEyeTrace().Entity

		if !(IsValid(entity) and entity:isDoor() and !entity:getNetVar("disabled")) then 
			client:notify("Look at a valid door.")
			return false
		end
		...
		if(IsValid(target) and target:getChar()) then
			local ownerTbl = {
				charID = target:getChar():getID(),
				name = target:Name(),
				doorName = "Owned Door",
				dur = tonumber(arguments[2]) or -1,
				set = os.date("%j"), --day of the year [001-365]
			}
			...
			client:notify("Door is now owned by " ..target:Name().. ".")
		
			entity:setNetVar("permaOwner", ownerTbl)
		end
...
nut.command.add("doorgetowner", {
	...
	onRun = function(client, arguments)
		...
		local permaOwner = entity:getNetVar("permaOwner")
		if(permaOwner) then
			client:notify("Door is owned by " ..permaOwner.name.. " for " ..permaOwner.dur.. " more days.")
		else
			client:notify("This door is not permanently owned.")
		end
	end
})

nut.command.add("doorextendowner", {
	syntax = "<number days>",
	onRun = function(client, arguments)
		-- Get the door the player is looking at.
		local entity = client:GetEyeTrace().Entity
		local addedDur = tonumber(arguments[1])
		...
		if(!addedDur) then
			client:notify("Specify a number of days.")
			return false
		end
		
		local dailyCost = entity:getNetVar("dailyCost", nut.config.get("doorCost", 10))
		if(!dailyCost) then
			client:notify("The door has no cost set.")
			return false
		end

		local cost = math.max(addedDur, 0) * dailyCost
		if(client:getChar():hasMoney(cost)) then
			client:getChar():takeMoney(cost)
			client:notify("You have spent " ..nut.currency.get(cost).. ".")
			
			local ownerTbl = entity:getNetVar("permaOwner", {})
			
			ownerTbl.charID = client:getChar():getID()
			ownerTbl.name = client:Name()
			ownerTbl.dur = (ownerTbl.dur or 0) + addedDur
			ownerTbl.day = os.date("%j")

			client:notify("Door duration extended by " ..addedDur.. " days.")
		
			entity:setNetVar("permaOwner", ownerTbl)
			
			PLUGIN:callOnDoorChildren(entity, function(child)
				child:setNetVar("permaOwner", ownerTbl)
			end)
			
			PLUGIN:SaveDoorData()
		else
			client:notify("You do not have enough money for this.")
		end
	end
})
...
nut.command.add("doorremoveowner", {
	...
	onRun = function(client, arguments)
		...
		client:notify("Door is now unowned.")

		entity:setNetVar("permaOwner", nil)
		
		PLUGIN:SaveDoorData()
	end
})

nut.command.add("doorsetcost", {
	adminOnly = true,
	syntax = "[number dailycost]",
	onRun = function(client, arguments)
		if(!tonumber(arguments[1])) then
			client:notify("Invalid cost specified")
			return false
		end
		...
		-- Set it so it is ownable.
		entity:setNetVar("dailyCost", tonumber(arguments[1]))

		PLUGIN:callOnDoorChildren(entity, function(child)
			child:setNetVar("dailyCost", tonumber(arguments[1]))
		end)

		client:notify("Daily cost increased to " ..tonumber(arguments[1]))
...
nut.command.add("doorsetcostall", {
	adminOnly = true,
	syntax = "[number dailycost]",
	onRun = function(client, arguments)
		if(!tonumber(arguments[1])) then
		...
		if (IsValid(entity) and entity:isDoor() and !entity:getNetVar("disabled")) then
			...
			elseif (client:IsAdmin()) then
				...
				PLUGIN:callOnDoorChildren(entity, function(child)
					...
					local childOwnerTbl = child:getNetVar("ownerTbl")
					if(childOwnerTbl) then
						childOwnerTbl.doorName = name
						
						child:setNetVar("permaOwner", childOwnerTbl)
					end
				end)
			else
