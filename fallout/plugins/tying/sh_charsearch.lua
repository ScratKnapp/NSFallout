...
if (SERVER) then
	...
	function PLUGIN:searchPlayer(client, target)
		...
		char:setData("equip", target:getEquip(), false, player.GetAll())
	...
	netstream.Hook("searchPly", function(target, index)
		...
		local equipPlugin = nut.plugin.list["equipment"]
		if(equipPlugin) then
			local targetEquip = target:getEquip()
		
			--augment menu
			local augment = vgui.Create("nutSlots")
			augment:createButtons("augment", augmentMat, equipPlugin.augSlots, targetEquip)
			augment:MoveLeftOf(storageInvPanel, 0)
			--augment:SetParent(panel)
			augment:ShowCloseButton(true)
			
			nut.gui.augment = augment
			
			--equipment menu
			local equip = vgui.Create("nutSlots")
			equip:createButtons("equipment", equipMat, equipPlugin.equipSlots, targetEquip)
			equip:MoveRightOf(localInvPanel, 0)
			--equip:SetParent(panel)
			equip:ShowCloseButton(true)
