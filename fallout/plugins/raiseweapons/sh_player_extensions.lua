...
if (SERVER) then
	...
	function playerMeta:toggleWepRaised()
		local timerName = "Raising..."
		
		if(self:isWepRaised()) then
			timerName = "Lowering..."
		end
		
		self:setAction("Raising...", 1.5, function()
			...
			if(name and name != "Scripted Weapon") then	
				if(self:isWepRaised()) then
					nut.chat.send(self, "me", "raises " ..weapon.PrintName.. ".")
				else
					nut.chat.send(self, "me", "lowers " ..weapon.PrintName.. ".")
				end
			end
			--]]
