...
do
	...
	function character:doesRecognize(id)
		if (id and type(id) != "number" and id.getID) then
			id = id:getID()
		end
...
if (CLIENT) then
	...
	function PLUGIN:GetDisplayedName(client, chatType)
		if (client != LocalPlayer()) then
			...
			local nickname = recogData[character:getID()]
		
			--if they're concealed by a mask or something
			if(client:getNetVar("concealed")) then
				--conceal system just changes name now so this happens
				return client:Name()
			end
...
nut.command.add("forcerecog", {
	adminOnly = true,
	onRun = function(client, arguments)
		local target = client:GetEyeTrace().Entity
		if (IsValid(target)) then
			target:getChar():recognize(client:getChar():getID())
			client:getChar():recognize(target:getChar():getID())
		else
			client:notify("You must be looking at a player.")
		end
	end
