...
function PANEL:configureSteps()
	self:addStep(vgui.Create("nutCharacterFaction"), 1)
	self:addStep(vgui.Create("nutCharacterModel"), 6)
	self:addStep(vgui.Create("nutCharacterBiography"), 7)
	hook.Run("ConfigureCharacterCreationSteps", self)
	
	local newSteps = {}
	local totalSteps = #self.steps
	
	for k, step in SortedPairsByMemberValue(self.steps, "priority") do
		newSteps[#newSteps+1] = step
	end
...
function PANEL:showError(message, ...)
	...
	self.error:SetTextColor(Color(255, 255, 255, 255))
	self.error:Dock(TOP)
