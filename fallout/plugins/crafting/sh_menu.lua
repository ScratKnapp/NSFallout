...
if(SERVER) then
	...
	netstream.Hook("nut_processSpecial", function(client, entity, recipe)
		...
		for k, v in SortedPairsByMemberValue(requiredItems, 1, false) do --basically goes for the least common ingredient first
			...
			for id, item in pairs(items, false) do	
				local tags = item:getData("tags", item.tags)
		...
		--[[
		for k, required in SortedPairsByValue(itemTable.items, true) do			
			count = 0
			
			for id, item in pairs(items, false) do	
				print(k)
				
				if(item.tags and item.tags[k]) then --an item's individual tags
					print(item, item.tags[k])
					if(count >= required) then
						break
					end
					...
					if(count > required) then
						...
						if(item.craft) then --ignore filler
							nameCount[name] = (nameCount[name] or 0) + quan - item.mark
					...
					if(item.craft) then
						for i = 1, (quan - (item.mark or 0)) do
							tableMerger(craftInfo, item.craft)
						end
					end
				end
			end
		...
		if(success) then
			...
			if(!itemTable.normResult) then
				...
				for k, v in pairs(itemTable.result) do
					...
					inventory:addSmart(k, v, client:getItemDropPos(), {creator = client:getChar():getID()})
					--PLUGIN:specCraftItem(client, k, itemName, craftInfo, level, itemTable) --generates the actual resulting item
				end
			end
	...
	function PLUGIN:specCraftItem(client, uniqueID, name, craftInfo, level, recipe)
		...
		local generatedData = nut.plugin.list["customization"]:generateLoot(level, uniqueID, true) or {}
