	...
	hook.Add("InitializedConfig", "nutChatTypes", function()
		...
		nut.chat.register("ic", {
			...
			onChatAdd = function(speaker, text, anonymous)
				...
				local chatFlip = NUT_CVAR_CHATFLIP and NUT_CVAR_CHATFLIP:GetBool()
				if suffix == "?" then
					if (chatFlip and math.random(1, 2) == 1) then
		...
		nut.chat.register("ooc", {
			...
			onChatAdd = function(speaker, text)
				...
				if(LocalPlayer():IsAdmin()) then
					chat.AddText(icon, Color(255, 50, 50), " [OOC] ", team.GetColor(speaker:Team()), speaker:Name().. " (" ..speaker:steamName().. ")", color_white, ": "..text)
				else
					chat.AddText(icon, Color(255, 50, 50), " [OOC] ", team.GetColor(speaker:Team()), speaker:Name(), color_white, ": "..text)
				end
			end,
			prefix = {"//", "/ooc"},
			noSpaceAfter = true,
