...
function PANEL:Init()
	...
	if(nut.plugin.list["randomname"]) then
		self.rando = self:Add("DButton")
		self.rando:Dock(TOP)
		self.rando:SetTall(36)
		self.rando:SetText("Randomize Name")
		self.rando:SetTextColor(nut.gui.character.WHITE)
		self.rando:SetFont("nutCharButtonFont")
		self.rando:DockMargin(0, 4, 0, 16)
		self.rando.DoClick = function()
			local randName = nut.plugin.list["randomname"]:generateName()
...
function PANEL:paintTextEntry(w, h)
	...
	surface.SetDrawColor(0, 47, 0, 100)
