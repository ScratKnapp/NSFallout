...
ITEM.functions._use = { 
	...
	onRun = function(item)
		...
		if (char and client:Alive()) then
			local actionID = item:getData("action", item.action)
			--local action = table.Copy(nut.plugin.list["spells"].spellList[actionID] or ACTI.actions[actionID])
			local action = table.Copy(ACTI.actions[actionID])
			
			if(action) then
				if(item.clientDMG) then
					local clientDMG = client:getDamage()
					action.dmg = clientDMG[1].dmg or 0
				end
				...
				if (trace.Hit) then
					nut.plugin.list["combat"]:attackStart(client, client, trace, action)
				end
			end
