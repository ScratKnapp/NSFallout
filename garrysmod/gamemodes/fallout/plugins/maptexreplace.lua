local PLUGIN = PLUGIN
PLUGIN.name = "Map Texture Replacement"
PLUGIN.author = "Ripped from Stormfox"
PLUGIN.desc = "Replaces ground textures with things when you want it to."

--[[
--sand : de_dust/groundsand03
--snow : nature/snowfloor001a
--]]

nut.config.add("texturereplacing", false, "Texture replacing enabled/disabled.", nil, {
	category = "Texture Replacements"
})

nut.config.add("replaceTex", "de_dust/groundsand03", "The font used to display titles.", nil, {
	category = "Texture Replacements"
})

PLUGIN._STORMFOX_ORIGNALTEX = PLUGIN._STORMFOX_ORIGNALTEX or {}
PLUGIN._STORMFOX_REPLACETEX_STR = PLUGIN._STORMFOX_REPLACETEX_STR or {}

-- Local functions
local tab = {
	["grass"] = {"gravelfloor004a","gravelfloor002a","mudfloor06a","blendgrassmud01","dirt","train_grass_floor_01","blendmilground008_4","milground0"},
...
local function LoadMapData()
	if file.Exists("stormfox/" .. game.GetMap() .. ".txt","data") then
		...
		PLUGIN._STORMFOX_ORIGNALTEX  = ScanMapTextures(materials)
	else
...
hook.Add("InitPostEntity","Stormfox_SandLoader", LoadMapData)
...
local function ReplaceMaterial(str, texture, id)
	...
	if not PLUGIN._STORMFOX_REPLACETEX_STR[str] then
		PLUGIN._STORMFOX_REPLACETEX_STR[str] = {}
	end
	
	if not PLUGIN._STORMFOX_REPLACETEX_STR[str][id] and (currentbase:GetName() or "null") != nut.config.get("replaceTex", "de_dust/groundsand03")  then
		PLUGIN._STORMFOX_REPLACETEX_STR[str][id] = currentbase:GetName()
	end
...
local function UndoAll()
	for mat,data in pairs(PLUGIN._STORMFOX_REPLACETEX_STR) do
		for id, str in pairs(data) do
			if str != nut.config.get("replaceTex", "de_dust/groundsand03") then
				--print("SNOW ERROR!")
...
local function MakeSnow(lvl)
	...
	local allowed = {}
	local ls = {"grass","roof","road"}
	for I = 1, 3 do
		allowed[ls[I]] = true
	end
	for _type,data in pairs(PLUGIN._STORMFOX_ORIGNALTEX ) do
		if allowed[_type] then
			--print(_type)
			for mat,textab in pairs(data) do
				for id,_ in pairs(textab) do
					--print(mat,tex,id)
					ReplaceMaterial(mat, nut.config.get("replaceTex", "de_dust/groundsand03"), id)
				end
			end
		end
	end
...
local l,lvl_old = 0,-1
local snowThink = CurTime() + 5

hook.Add("Think","Stormfox_SandReplacement",function()
	if(snowThink < CurTime()) then
		snowThink = CurTime() + 10
		local con = nut.config.get("texturereplacing", false)
		if (con) then
			MakeSnow(1)
			MakeSnow(1)
		else
...
hook.Add("PlayerFootstep","Stormfox_SandReplacementSounds",function( ply, pos, foot, sound, volume, rf )
	if(!CLIENT) then
		return
	end
	local lvl = math.Clamp(1,0,3)
	if lvl <= 0 then return end
	if table.Count(PLUGIN._STORMFOX_REPLACETEX_STR) <= 0 then return end
	...
	if PLUGIN._STORMFOX_REPLACETEX_STR[mat_name] then
		ply:EmitSound( "player/footsteps/snow" .. math.random(1,6) .. ".wav" )
		return true
	end
	
	if mat:GetTexture("$basetexture") then
		if mat:GetTexture("$basetexture"):GetName() == nut.config.get("replaceTex", "de_dust/groundsand03") then
			ply:EmitSound( "player/footsteps/snow" .. math.random(1,6) .. ".wav" )
			return true
		end
	end
	...
	if mat:GetTexture("$basetexture2") then
		if mat:GetTexture("$basetexture2"):GetName() == nut.config.get("replaceTex", "de_dust/groundsand03") then
			ply:EmitSound( "player/footsteps/snow" .. math.random(1,6) .. ".wav" )
			return true
		end
	end
