ITEM.name = "Melee Base Item (Stun)"
ITEM.desc = "<color=255,255,255>[Colorful Code Here]</color> An unarmed melee weapon with the Stun ability. (Base, customize afterwards.)"
ITEM.model = "models/mosi/fallout4/props/weapons/melee/baseballbat.mdl"
ITEM.width = 1
ITEM.height = 1
ITEM.iconCam = {
	pos = Vector(15, 200, -4),
	ang = Angle(0, 270, 0),
	fov = 4,
	outline = true,
	outlineColor = Color(90, 90, 90),
}
ITEM.specialSlot = "Sidearm"
ITEM.category = "Weapon - Melee"
ITEM.durability = 500
ITEM.price = 50
 

ITEM.weight = 1
ITEM.weapondual = false
ITEM.costAP = 1 --how much AP is used when using this weapon normally
ITEM.accuracy = 1 --bonus accuracy, can also go negative. defaults to 0 if not set
ITEM.class = "aus_m_fists_boxingglove"

ITEM.dmg = {
	["Slash"] = 15
}

--long, medium, close
--the number in here determines the affects on accuracy at that range 
-- -1 in the first spot will reduce accuracy at long range by 1
-- 1 in the last spot will increase accuracy at close range by 1
ITEM.range = {-100,-100,-100,1}-- arccw_bo1_makarov

ITEM.skillScaleAcc = {
["unarmed"] = 1.5,}

ITEM.skillScaleDmg = {
["unarmed"] = 1.5,}

ITEM.actions = {	
"charge",
"stun",
"heavyattack",

}

--use this if you want things sto scale off of stats
--[[
ITEM.scaling = { --added to damage calculation
	["stm"] = 0.2,
	["talent"] = 0.2,
}
--]]