	...
	onRun = function(client, arguments)
		if (arguments[1]) then
			if (!string.find(client:Name(), "'")) then
				...
				local newName = (name[1] or "").." '"..arguments[1].."' "..(name[2] or "")
...
nut.command.add("charaddmoney", {
	adminOnly = true,
	syntax = "<string name> <number money>",
	onRun = function(client, arguments)
		...
		if(IsValid(target) and target:getChar()) then
			...
			client:notify("You have given " ..target:Name().. " " ..nut.currency.get(money).. ".")
			target:notify("You have received " ..nut.currency.get(money).. ".")
		end
	end
})

--i usually just use this for testing desc gui things with bots
--[[
...
nut.command.add("rolld", {
	...
	onRun = function(client, arguments)
		...
		if(bonus) then
			bonus = math.Clamp(bonus, -1000000, 1000000)
		end
		...
		nut.log.addRaw(client:Name().. " " ..msg)
	end
})

nut.command.add("rolla", {
	adminOnly = true,
	syntax = "<number roll>",
	onRun = function(client, arguments)
		local roll = tonumber(arguments[1])
		if(!roll) then
			client:notify("Specify a number.")
			return false
		end
		...
		nut.chat.send(client, "rolld", msg)
		nut.log.addRaw(client:Name().. "[FAKE] " ..msg)
	end
})

nut.command.add("rollloaded", {
	adminOnly = true,
	syntax = "<string name> <number roll>",
	onRun = function(client, arguments)
		local roll = tonumber(arguments[2])
		if(!roll) then
			client:notify("Specify a number.")
			return false
		end
		...
		local target = nut.command.findPlayer(client, arguments[1])
		if(IsValid(target) and target:getChar()) then
			target.rollLoaded = roll
			client:notify("Successfully loaded roll.")
		end
	end
})
...
nut.command.add("forums", {
	onRun = function(client, arguments)
		client:SendLua([[gui.OpenURL("https://newlightrp.boards.net/")]])
	end
})

nut.command.add("content", {
	onRun = function(client, arguments)
		client:SendLua([[gui.OpenURL("https://steamcommunity.com/sharedfiles/filedetails/?id=3218839275")]])
	end
})
...
nut.command.add("getmodel", {
    syntax = "",
    onRun = function(client, arguments)
        local targetEnt = client:GetEyeTrace().Entity
		
		if(IsValid(targetEnt) and targetEnt.GetModel) then
			client:notify(targetEnt:GetModel())
		end
    end
})