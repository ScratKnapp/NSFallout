...
function SWEP:GetBobbyPins()
	...
	if(itemID) then
		...
		if(item) then
			lockpicks = item:getData("Amount", 1)
		end
	end
...
function SWEP:SetBobbyPins(Amount) 
	local itemID = self.item
	if(itemID) then
		local item = nut.item.instances[itemID]
	...
	net.Receive("lockpick_break", function(len, ply)
		if ply:HasWeapon("lockpick") then
			...
			if wep:GetClass() == "lockpick" then
				wep:SetBobbyPins(wep:GetBobbyPins() - 1) 
				wep:Fail()
				wep:SetNextPrimaryFire(CurTime() + 1)
				ply:ChatPrint("You broke a bobby pin!")
			end
		end
	end)
...
function SWEP:PrimaryAttack()
	...
	local totalLockpicks = self:GetBobbyPins()
	if totalLockpicks <= 0 then
		self.Owner:ChatPrint("You have no bobby pins left!")
		return
	end
	...
	local canLockpick = hook.Run("CanLockpickEntity", self.Owner, ent, self)
...
function SWEP:Succeed()
	...
	hook.Run("OnLockpickEntity", self.Owner, ent, self)
...
function SWEP:SecondaryAttack()
	self.Owner:ChatPrint( "You have "..self:GetBobbyPins().." bobby pin(s) left!" )
end