...
function ENT:HiderEject()
	local client = self.hider
	if(IsValid(client)) then	
		client:SetNoDraw(false)
		client:SetNotSolid(false)
		client:SetMoveType(MOVETYPE_WALK)
		
		client.storageHide = nil
		self.hider = nil
		
		client:notify("You are no longer hiding in " ..(self:getNetVar("name") or "Storage").. ".")
	end
end

function ENT:HiderEnter(client)
	local trace = client:GetEyeTraceNoCursor()

	if(client.storageHide) then --get out
		self:HiderEject()
		return
	end
	...
	local w = inventory:getWidth()
	local h = inventory:getHeight()

	if(w < 4 and h < 4) then
		client:notify("This storage is too small to hide in.")
		return false
	end
	...
	client:notify("You are now hiding in " ..(self:getNetVar("name") or "Storage").. ".")
end

netstream.Hook("nut_hideInside", function(client, entity)
	local distance = client:GetPos():Distance(entity:GetPos())
	if(distance < 200) then --so people don't abuse the hook or whatever (not as easily anyways)
		entity:HiderEnter(client)
	end
end)