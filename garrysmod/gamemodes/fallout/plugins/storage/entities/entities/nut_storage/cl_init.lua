...
--the hide prompt thing
function PLUGIN:OnCreateStoragePanel(localInvPanel, storageInvPanel, storage)
	--[[
	local frame = vgui.Create("DFrame", localInvPanel)
	frame:SetSize(150, 75)
	frame:MakePopup()
	frame:SetTitle("")
	frame:MoveAbove(storageInvPanel, 8)
	frame:MoveLeftOf(localInvPanel, 8)
	
	local button = vgui.Create("DButton", frame)
	button:Dock(FILL)
	button:SetText("Hide Inside")
	button.DoClick = function()
		netstream.Start("nut_hideInside", storage)
		localInvPanel:Close()
	end
	--]]
end