ITEM.name = "Placeable Base"
ITEM.desc = ""
ITEM.model = "models/props_junk/gnome.mdl"
ITEM.category = "Placeable Objects"
ITEM.width = 1
ITEM.height = 1
ITEM.flag = "V"
...
ITEM.functions.Place = {
	...
	onRun = function(item)
		...
		if(!IsValid(weapon)) then
			weapon = client:Give("nut_entityspawner")
		end
		
		client:SelectWeapon("nut_entityspawner")
		
		if(IsValid(weapon)) then
			weapon.item = item
			weapon:SetNW2String("model", item.model)
		end
...
ITEM.functions.CustomColor = {
	...
	onRun = function(item)
		...
		netstream.Start(client, "nut_entity_color", item.id, color, skin, model)
...
if(SERVER) then
	netstream.Hook("nut_entity_colorF", function(client, itemID, color, skin)
		if(itemID) then
	...
	netstream.Hook("nut_entity_color", function(itemID, color, skin, modelPath)
		...
		finishB.DoClick = function()
			...
			netstream.Start("nut_entity_colorF", itemID, customColor, customSkin)
