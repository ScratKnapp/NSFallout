...
if (CLIENT) then
	...
	function PLUGIN:PlayerBindPress(client, bind, pressed)
		...
		elseif (bind:find("attack") and self.alpha > 0) then
			...
			netstream.Start("nut_weaponSwitch", weapon)
...
if(SERVER) then
	...
	netstream.Hook("nut_weaponSwitch", function(client, weapon)
		--code for printing message in chat on weapon swap
		local weapon1 = false --whether or not the weapon held is a nonweapon
		local weapon2 = false --whether or not the weapon to swap to is a nonweapon

		local curWeapon = client:GetActiveWeapon()
		if(nonWeapon[curWeapon:GetClass()]) then
			weapon1 = true
		end
		
		local newWeapon = weapon
		if(nonWeapon[newWeapon:GetClass()]) then
			weapon2 = true
		end

		--from non weapon to weapon
		if(weapon1 and !weapon2) then
			--grab the weapon table so we can get its name
			local name = hook.Run("GetWeaponName", newWeapon) or newWeapon.PrintName or "their weapon"

			--nut.chat.send(client, "me", "draws " ..name.. ".")
		elseif(!weapon1 and weapon2) then --from weapon to non weapon
			local name = hook.Run("GetWeaponName", curWeapon) or curWeapon.PrintName or "their weapon"
		
			--nut.chat.send(client, "me", "stows " ..name.. ".")
		end
	end)
end