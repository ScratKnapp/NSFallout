			...
			if (char) then
				for k, v in pairs(nut.attribs.list) do
					if (nut.util.stringMatches(L(v.name, client), attribName) or nut.util.stringMatches(k, attribName)) then
						...
						client:notifyLocalized("attribSet", target:Name(), L(v.name, client), attribNumber)
	...
	onRun = function(client, arguments)
		...
		if (IsValid(target)) then
			...
			if (char) then
				for k, v in pairs(nut.attribs.list) do
					if (nut.util.stringMatches(L(v.name, client), attribName) or nut.util.stringMatches(k, attribName)) then
						char:updateAttrib(k, attribNumber)
						client:notifyLocalized("attribUpdate", target:Name(), L(v.name, client), attribNumber)
