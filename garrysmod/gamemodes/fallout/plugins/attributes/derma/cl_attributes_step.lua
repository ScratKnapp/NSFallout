...
function PANEL:Init()
	self.title = self:addLabel("SPECIAL")
	self.leftLabel = self:addLabel("points left")
	self.leftLabel:SetFont("nutCharSubTitleFont")

	self.raceBonus = self:getRaceBonus()

	self.max = nut.config.get("maxAttribs", 10)
	
	self.total = hook.Run(
		"GetStartAttribPoints",
		LocalPlayer(),
		self:getContext()
	) or nut.config.get("startAttribs", 28)
	
	self.attribs = self.attribs or {}
...
function PANEL:getRaceBonus()
	local bonusTbl = {}
	
	local race = self:getContext("faction")
	if(race) then
		local raceTbl = nut.faction.indices[race]
		if(raceTbl.attrib) then
			bonusTbl = raceTbl.attrib
		end
	end
...
function PANEL:defaultAttribs()
	self.left = self.total

	local attribCount = table.Count(nut.attribs.list)

	if(!table.IsEmpty(self.attribs)) then
	...
	for k, v in SortedPairsByMemberValue(nut.attribs.list, "name") do
		...
		self.attribs[k]:delta(math.floor(self.total/attribCount))
	end
	
	return self:getContext("attribs", {})
end

function PANEL:onDisplay()
	local attribs = self:getContext("attribs", {})
	
	if(table.IsEmpty(attribs)) then
	...
	for key, row in pairs(self.attribs) do
		...
		row.bonus = self.raceBonus[key] or 0
		row:updateQuantity()
	end
	
	self:setContext("ptAttrib", self.left)
end

function PANEL:addAttribute(key, attribute)
	local row = self:Add("nutCharacterAttribsRow")
	row:setAttribute(key, attribute, self.raceBonus[key])
	row.parent = self
	return row
end
...
function PANEL:onPointChange(key, delta)
	...
	if (
		...
		newQuantity < 1 or newQuantity > self.max
	) then
		return quantity
	end
...
function PANEL:setAttribute(key, attribute, bonus)
	self.key = key
...
function PANEL:updateQuantity()
	...
	if(self.bonus and self.bonus != 0) then
		local add = "+"
		if(self.bonus < 0) then
			add = ""
		end
		
		text = text..add..math.floor(self.bonus)
	end
