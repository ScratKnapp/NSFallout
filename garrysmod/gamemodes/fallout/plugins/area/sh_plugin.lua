...
if (SERVER) then
	...
	timer.Create("nutAreaController", 0.33, 0, function()
		for k, v in ipairs(player.GetAll()) do
			...
			if (char and v:Alive()) then
				...
				if(!inArea) then
					v.curArea = nil
					hook.Run("OnPlayerAreaChanged", v)
				end
			end
		end
	end)
	...
	function PLUGIN:OnPlayerAreaChanged(client, areaID)
		if(areaID) then -- Entered area
			local areaData = nut.area.getArea(areaID)
			netstream.Start(client, "displayAreaText", tostring(areaData.name))

			if(areaData.music) then
				PLUGIN:areaMusic(client, areaData.music)
				client.areaMusic = true
			end
		else -- Exited area
			if(client.areaMusic) then
				PLUGIN:areaMusicStop(client)
				client.areaMusic = nil
			end
		end
	end
	
	--caches sound durations so we dont have to check more than once
	PLUGIN.soundDurCache = {}
	
	function PLUGIN:areaMusic(client, musicData)
		for k, sound in pairs(musicData) do
			local duration = PLUGIN.soundDurCache[sound.path] or NewSoundDuration(sound.path)
		
			sound.duration = duration
			PLUGIN.soundDurCache[sound.path] = duration
		end
		
		-- send data to client
		netstream.Start(client, "area_musicStart", musicData)
	end
	
	function PLUGIN:areaMusicStop(client)
		-- send data to client
		netstream.Start(client, "area_musicStop")
	end
	...
	netstream.Hook("areaUpdateMusic", function(client, areaID, areaData)
		-- Only Admin can edit the area.
		if (!client:IsAdmin()) then
			return false
		end
		...
		if (areaData) then
			...
			PLUGIN.areaTable[areaID] = areaData
			PLUGIN:saveAreas()
		end
	end)
	...
	function PLUGIN:Think()
		...
		if(client.areaMusic) then
			for k, sound in pairs(client.areaMusic) do
				-- this is for looping
				if(sound.endTime and sound.endTime < CurTime()) then
					local volume = sound.volume or 1
				
					client.areaMusic[k].song:Stop()
				
					client.areaMusic[k].song = CreateSound(client, sound.path)
					client.areaMusic[k].song:PlayEx(volume, sound.pitch or 100)
					
					client.areaMusic[k].endTime = CurTime() + sound.duration
					client.areaMusic[k].startTime = CurTime()
				end
			end
		end
	end

	--starts music for the client
	function PLUGIN:areaMusicStart(musicData)
		local client = LocalPlayer()
	
		PLUGIN:areaMusicStop()
	
		--local volume = math.Round(NUT_CVAR_TURNVOL and NUT_CVAR_TURNVOL:GetFloat() or 1, 2)
	
		client.areaMusic = client.areaMusic or {}
	
		for k, sound in pairs(musicData) do
			local volume = sound.volume or 1

			client.areaMusic[k] = client.areaMusic[k] or {}
		
			client.areaMusic[k].song = CreateSound(client, sound.path)
			client.areaMusic[k].song:PlayEx(0.01, sound.pitch or 100)
			client.areaMusic[k].song:ChangeVolume(volume, 4) --fade in
			
			client.areaMusic[k].path = sound.path
			client.areaMusic[k].pitch = sound.pitch
			client.areaMusic[k].volume = sound.volume
			client.areaMusic[k].endTime = CurTime() + sound.duration
			client.areaMusic[k].startTime = CurTime()
			client.areaMusic[k].duration = sound.duration
		end
	end
	
	--stops combat music that is currently playing for the client
	function PLUGIN:areaMusicStop()
		local client = LocalPlayer()

		for k, area in pairs(client.areaMusic or {}) do
			if(area and area.song and area.song:IsPlaying()) then
				area.song:FadeOut(4)
			end
		end
	...
	netstream.Hook("area_musicStart", function(musicData)
		PLUGIN:areaMusicStart(musicData)
	end)
	
	netstream.Hook("area_musicStop", function()
		PLUGIN:areaMusicStop()
	end)
	...
	function PLUGIN:HUDPaint()
		...
		for i = 1, math.min(maxDisplay, strEnd) do
			...
			drawMatrixString(char,
				...
				math.Round((h * 0.15) - (sy or 0)*scale/2),
...
--[[
nut.command.add("areamusic", {
	adminOnly = true,
	onRun = function(client, arguments)
		netstream.Start(client, "nutAreaMusic", nut.area.getAllArea())
	end
})
--]]