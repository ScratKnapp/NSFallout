local PANEL = {}
	local background = Material("fonvui/backgroundimage.png")
	local header = Material("header_barebones.png")
	...
	function PANEL:addTab(name, callback, uniqueID)
		...
		local function paintTab(tab, w, h)
			if (self.activeTab == tab) then
				surface.SetDrawColor(Color(0, 238, 0, 200))
				surface.DrawRect(0, h - 8, w, 8)
			elseif (tab.Hovered) then
				surface.SetDrawColor(0, 238, 0, 50)
				surface.DrawRect(0, h - 8, w, 8)
			end
		end
		...
		local tab = self.tabs:Add("DButton")
			...
			tab:SetTextColor(Color(0, 238, 0, 255))
