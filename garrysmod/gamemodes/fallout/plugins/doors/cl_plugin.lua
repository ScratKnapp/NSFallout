...
function PLUGIN:DrawEntityInfo(entity, alpha)
	if (entity.isDoor(entity) and !entity:getNetVar("hidden") and hook.Run("CanDrawDoorInfo") != false) then
		local ownerTbl = entity:getNetVar("permaOwner") or {}
	
		local position = toScreen(entity.LocalToWorld(entity, entity.OBBCenter(entity)))
		local x, y = position.x, position.y
		local owner = entity.GetDTEntity(entity, 0)
		local name = ownerTbl.doorName or entity.getNetVar(entity, "title", entity.getNetVar(entity, "name", IsValid(owner) and L"dTitleOwned" or L"dTitle"))
		...
		if(ownerTbl.name) then
			--drawText(L("dOwnedBy", ownerTbl.name), x, y + 16, colorAlpha(color_white, alpha), 1, 1)
