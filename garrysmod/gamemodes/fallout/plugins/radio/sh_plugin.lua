...
local function hasRadio(client, findFreq)
		...
		for k, v in pairs(inventory:getItems()) do
			...
			for id, _ in pairs(find) do
				if (v.uniqueID == id and v:getData("power", false) == true) then
					if(findFreq) then
						if(findFreq == v:getData("freq", "000.0")) then
							return findFreq
						end
					else
	...
	local equipment = client:getEquip()
	if(equipment) then
...
nut.chat.register("radio", {
	...
	onCanHear = function(speaker, listener)
		...
		local radioFreq = hasRadio(listener, CURFREQ)
		if(radioFreq) then
			endChatter(listener)
			return true
		end
	...
	onCanSay = function(speaker, text)
		...
		local radioFreq = hasRadio(speaker)
		if(radioFreq) then
			freq = radioFreq
		end
