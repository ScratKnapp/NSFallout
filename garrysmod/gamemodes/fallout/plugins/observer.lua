...
if (CLIENT) then
	...
	function PLUGIN:HUDPaint()
		...
		if (client:IsAdmin() and client:GetMoveType() == MOVETYPE_NOCLIP and !client:InVehicle() and NUT_CVAR_ADMINESP:GetBool()) then
			...
			if((HP) and NUT_CVAR_ADMINESPR:GetBool()) then
				for k, v in ipairs(player.GetAll()) do
					if (v == client) then continue end
				
					scrPos = v:GetPos():ToScreen()
					marginx, marginy = sy*.1, sy*.1
					x, y = scrPos.x, scrPos.y + 15
					distance = client:GetPos():Distance(v:GetPos())
					factor = 1 - math.Clamp(distance/dimDistance, 0, 1)
					size = math.max(10, 32*factor)
					alpha = math.Clamp(255*factor, 80, 255)
					...
					if(HP[v]) then
						nut.util.drawText("(" .. HP[v][1] .. "/" ..HP[v][2].. ")", x, y - size, ColorAlpha(Color(200,75,75), alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha)
					end
				end
			end			
			...
			if(NUT_CVAR_ADMINESPC:GetBool()) then
				for k, v in ipairs(ents.GetAll()) do
					if(!v.combat or !IsValid(v)) then continue end
					...
					distance = client:GetPos():Distance(v:GetPos())
					factor = 1 - math.Clamp(distance/dimDistance, 0, 1)
					local teamColor = Color(255,90,0)
					
					size = math.max(10, 32*factor)
					alpha = math.Clamp(255*factor, 100, 255)
			...
			if(NUT_CVAR_ADMINESPI:GetBool()) then
				for k, v in pairs(ents.FindByClass("nut_item")) do
					...
					if(item) then
						...
						nut.util.drawText((item.getName and item:getName()) or item.name or "Item", x, y - size, ColorAlpha(Color(255,255,255), alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha)
					end
				end
			end
		end
	...
	function PLUGIN:SetupQuickMenu(menu)
		if (LocalPlayer():IsAdmin()) then
			...
			if(MANA and HP) then
				local buttonESPR = menu:addCheck("Toggle MP/HP ESP", function(panel, state)
					if (state) then
						RunConsoleCommand("nut_obsespr", "1")
					else
						RunConsoleCommand("nut_obsespr", "0")
					end
				end, NUT_CVAR_ADMINESPR:GetBool())
			end
			...
			local buttonESPI = menu:addCheck("Toggle Item ESP", function(panel, state)
				if (state) then
	...
	function PLUGIN:PlayerNoClip(client, state)
		...
		if (client:IsAdmin()) then
			...
			if (state) then
				...
				client:GodEnable()
				client:SetNoTarget(true) --makes npcs not target them
