...
function PANEL:canCreateCharacter()
	...
	for k, v in pairs(nut.faction.teams) do
		if nut.faction.hasWhitelist(v.index) then validFactions[#validFactions + 1] = v.index end
	end
	...
	if nut.characters and #nut.characters >= maxChars then return false, "You have reached the maximum number of characters" end
...
function PANEL:onFinish()
	...
	nutMultiChar:createCharacter(self.context):next(function()
		onResponse()
		if IsValid(nut.gui.character) then nut.gui.character:showContent() end
	end, onFail)
...
function PANEL:Init()
	...
	self.content:SetWide(800)
	self.content:SetTall(700)
	timer.Simple(FrameTime(), function() if IsValid(self.content) then self.content:Center() end end)
	self.content:DockMargin(sideMargin, 0, sideMargin, 0)
