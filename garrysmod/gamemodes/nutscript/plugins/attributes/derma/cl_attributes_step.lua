...
function PANEL:Init()
	...
	self.leftLabel:SetFont("CYR_ATTRIBUTETitle")
	self.total = hook.Run("GetStartAttribPoints", LocalPlayer(), self:getContext()) or nut.config.get("maxAttribs", 30)
	self.attribs = {}
...
function PANEL:delta(delta)
	if IsValid(self.parent) then
		...
		self:updateQuantity()
		if oldPoints ~= self.points then LocalPlayer():EmitSound(unpack(SOUND_ATTRIBUTE_BUTTON)) end
	end
end
