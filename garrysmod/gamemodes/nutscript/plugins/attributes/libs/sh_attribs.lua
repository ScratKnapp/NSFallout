...
function nut.attribs.loadFromDir(directory)
	for _, v in ipairs(file.Find(directory .. "/*.lua", "LUA")) do
		...
		if PLUGIN then ATTRIBUTE.plugin = PLUGIN.uniqueID end
...
do
	...
	if SERVER then
		...
		function charMeta:addBoost(boostID, attribID, boostAmount)
			...
			print(string.format("[Boost] %s Added %s to %s: %s", self:getName(), boostID, attribID, boostAmount))
		...
		function charMeta:removeBoost(boostID, attribID)
			...
			print(string.format("[Boost] %s Removed %s from %s", self:getName(), boostID, attribID))
			return self:setVar("boosts", boosts, nil, self:getPlayer())
		end
	else
