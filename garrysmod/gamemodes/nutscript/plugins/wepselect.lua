...
if CLIENT then
	...
	-- convarf for inverting the weapon selection
	local invertWeaponSelect = CreateClientConVar("CYR_weaponselect_invert", "0", true, false, "Inverts the weapon selection direction.")
	...
	function PLUGIN:HUDPaint()
		...
		for _, v in pairs(weapons) do
			...
			local color = ColorAlpha(realIndex == self.index and nut.config.get("color") or color_white, (255 - math.abs(theta * 3) * 255) * fraction)
			local lastY = 0
			...
			nut.util.drawText(name, 2, ty / 2, color, 0, 1, "nutSubTitleFont")
			cam.PopModelMatrix()
	...
	function PLUGIN:onIndexChanged()
		...
		if IsValid(weapon) then
			...
			if text ~= "" then self.markup = markup.Parse("<font=nutItemDescFont>" .. text, ScrW() * 0.3) end
	...
	function PLUGIN:PlayerBindPress(client, bind, pressed)
		...
		bind = bind:lower()
		local mp = invertWeaponSelect:GetBool()
		local invPrev = mp and "invnext" or "invprev"
		local invNext = mp and "invprev" or "invnext"
		local total = table.Count(client:GetWeapons())
		if bind:find(invPrev) then
			if hook.Run("CanPlayerChooseWeapon") == false then return true end
...
function PLUGIN:StartCommand(client, cmd)
	if hook.Run("ShouldDoStartCommand", client, cmd) == false then return end
