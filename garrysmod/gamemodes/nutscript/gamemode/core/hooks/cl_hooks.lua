...
function GM:CalcView(client, origin, angles, fov)
	...
	if (not client:ShouldDrawLocalPlayer() and IsValid(entity) and entity:IsRagdoll()) or (not LocalPlayer():Alive() and IsValid(ragdoll)) then -- First person if the player has fallen over. -- Also first person if the player is dead.
...
function GM:HUDPaintBackground()
	...
	if blurValue > 0 and not localPlayer:ShouldDrawLocalPlayer() then nut.util.drawBlurAt(0, 0, scrW, scrH, blurValue) end
...
function GM:PlayerBindPress(client, bind, pressed)
	...
	if (bind:find("use") or bind:find("attack")) and pressed then
		...
		elseif bind:find("use") and pressed then
			...
			if IsValid(entity) and (entity:GetClass() == "nut_item" or entity.hasMenu == true) then hook.Run("ItemShowEntityMenu", entity) end
		end
...
function GM:ItemShowEntityMenu(entity)
	...
	if not itemTable then -- MARK: This is the where error came from.
		return
	end
	...
	if input.IsShiftDown() or true then callback("take") end
	for k, v in SortedPairs(itemTable.functions) do
		if k == "combine" then -- yeah, noob protection
			continue
		end
	...
	if table.Count(options) > 0 then entity.nutMenuIndex = nut.menu.add(options, entity) end
