nut.command.add("roll", {
	...
	onRun = function(client, arguments) nut.chat.send(client, "roll", math.random(0, math.min(tonumber(arguments[1]) or 100, 100))) end
})
	...
	onRun = function(client, arguments)
		...
		if IsValid(target) and target:getChar() then
			...
			if not flags then
				...
				return client:requestString("@flagGiveTitle", "@flagGiveDesc", function(text) nut.command.run(client, "flaggive", {target:Name(), text}) end, available)
			end
...
nut.command.add("flagtake", {
	...
	onRun = function(client, arguments)
		...
		if IsValid(target) and target:getChar() then
			local flags = arguments[2]
			if not flags then return client:requestString("@flagTakeTitle", "@flagTakeDesc", function(text) nut.command.run(client, "flagtake", {target:Name(), text}) end, target:getChar():getFlags()) end
...
nut.command.add("charsetname", {
	...
	onRun = function(client, arguments)
		local target = nut.command.findPlayer(client, arguments[1])
		if IsValid(target) and not arguments[2] then return client:requestString("@chgName", "@chgNameDesc", function(text) nut.command.run(client, "charsetname", {target:Name(), text}) end, target:Name()) end
	...
	onRun = function(client, arguments)
		...
		if IsValid(target) and target:getChar() then
			...
			target:getChar():getInv():add(uniqueID, amount or 1):next(function(res)
				if IsValid(target) then target:notifyLocalized("itemCreated") end
				if IsValid(client) and client ~= target then client:notifyLocalized("itemCreated") end
				hook.Run("CharGivenItem", target, res)
			end):catch(function(err) if IsValid(client) then client:notifyLocalized(err) end end)
		end
	end
})
...
nut.command.add("charunban", {
	...
	adminOnly = true,
	onRun = function(client, arguments)
		if (client.nutNextSearch or 0) >= CurTime() then return L("charSearching", client) end
...
nut.command.add("fallover", {
	...
	onRun = function(client, arguments)
		...
		if not IsValid(client.nutRagdoll) then client:setRagdolled(true, time) end
	end
})
...
nut.command.add("chardesc", {
	...
	onRun = function(client, arguments)
		...
		if not arguments:find("%S") then return client:requestString("@chgDesc", "@chgDescDesc", function(text) nut.command.run(client, "chardesc", {text}) end, client:getChar():getDesc()) end
...
nut.command.add("plytransfer", {
	...
	onRun = function(client, arguments)
		...
		if faction.onTransfered then faction:onTransfered(target, oldFaction) end
...
nut.command.add("content", {
	onRun = function(client) client:SendLua([[gui.OpenURL(nut.config.get("contentURL", "https://nutscript.net"))]]) end
})
