...
if not nut.command then include("sh_command.lua") end
-- Returns a timestamp
	...
	if CLIENT and data.prefix then
		if type(data.prefix) == "table" then
			for _, v in ipairs(data.prefix) do
				if v:sub(1, 1) == "/" then nut.command.add(v:sub(2), DUMMY_COMMAND) end
			end
		else
...
function nut.chat.parse(client, message, noSend)
	...
	for k, v in pairs(nut.chat.classes) do
		...
		if isChosen then
			...
			if nut.chat.classes[k].noSpaceAfter and message:sub(1, 1):match("%s") then message = message:sub(2) end
			break
		end
	end
	...
	hook.Add("InitializedConfig", "nutChatTypes", function()
		...
		nut.chat.register("ic", {
			...
			onGetColor = function(speaker, text)
				...
				if LocalPlayer():GetEyeTrace().Entity == speaker then return nut.config.get("chatListenColor") end
		...
		nut.chat.register("it", {
			onChatAdd = function(speaker, text) chat.AddText(nut.chat.timestamp(false), nut.config.get("chatColor"), "**" .. text) end,
			radius = function() return nut.config.get("chatRange", 280) end,
			prefix = {"/it"},
			font = "nutChatFontItalics",
			filter = "actions",
		...
		nut.chat.register("w", {
			...
			radius = function() return nut.config.get("chatRange", 280) * 0.25 end,
			prefix = {"/w", "/whisper"}
		})
		...
		nut.chat.register("y", {
			...
			radius = function() return nut.config.get("chatRange", 280) * 2 end,
			prefix = {"/y", "/yell"}
		})
		...
		nut.chat.register("ooc", {
			...
			onChatAdd = function(speaker, text)
				...
				if (nut.config.get("oocLimit", 0) ~= 0) and (#text > nut.config.get("oocLimit", 0)) then text = string.sub(text, 1, nut.config.get("oocLimit", 0)) .. "..." end
...
local color_orange = Color(259, 150, 0)
-- Global events.
nut.chat.register("event", {
	onCanSay = function(speaker, text) return speaker:IsAdmin() end,
	onChatAdd = function(speaker, text) chat.AddText(nut.chat.timestamp(false), color_orange, text) end,
	prefix = {"/event"}
})
