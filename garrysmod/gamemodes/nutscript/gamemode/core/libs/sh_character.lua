...
if SERVER then
	if not nut.db then include("sv_database.lua") end
	...
	netstream.Hook("nutCharFetchNames", function(client) netstream.Start(client, "nutCharFetchNames", nut.char.names) end)
...
if CLIENT then
	...
	netstream.Hook("nutCharFetchNames", function(data) nut.char.names = data end)
	if #nut.char.names < 1 then netstream.Start("nutCharFetchNames") end
end
...
do
	...
	nut.char.registerVar("faction", {
		...
		onValidate = function(value, data, client)
			if not nut.faction.indices[value] then return false, "invalid", "faction" end
			if not client:hasWhitelist(value) then return false, "illegalAccess" end
			return true
		end,
		onAdjust = function(client, data, value, newData) newData.faction = nut.faction.indices[value].uniqueID end
	})
