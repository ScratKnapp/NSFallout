...
if SERVER then
	...
	function ENT:OnRemove()
		...
		if not nut.shuttingDown and not self.nutIsSafe and self.nutItemID then nut.item.deleteByID(self.nutItemID) end
	end
	...
	function ENT:computeDescMarkup(description)
		if self.desc ~= description then
			...
			self.markup = nut.markup.parse("<font=nutItemDescFont>" .. description .. "</font>", ScrW() * 0.5)
		end
		return self.markup
	end
	...
	function ENT:onDrawEntityInfo(alpha)
		...
		nut.util.drawText(L(itemTable.getName and itemTable:getName() or itemTable.name), x, y, colorAlpha(nut.config.get("color"), alpha), 1, 1, nil, alpha * 0.65)
		y = y + 12
